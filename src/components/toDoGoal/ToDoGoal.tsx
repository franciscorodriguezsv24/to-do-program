    ...
    filteredData.map((task:TodoElementsMock, index) => {
      return (
        ...
        <div className={styles.toDoElementsLeft}>
          <p className={styles.toDoElementsLeftTitle}>{pendingTasks().length} items left</p>
          <button className={styles.toDoFooterButton}onClick={() => deleteElementsTest()}>Clear Completed</button>
        </div>
  </div>


  <div className={styles.toDoFooter}>
      <button className={filterStatus === 'All' ? styles.filterActive : styles.toDoFooterButton} onClick={() => setFilterStatus('All')}>
          All
      </button>
      <button className={filterStatus === 'Active' ? styles.filterActive : styles.toDoFooterButton } onClick={() => setFilterStatus('Active')}>
          Active
      </button>
      <button className={filterStatus === 'Completed' ? styles.filterActive : styles.toDoFooterButton} onClick={() => setFilterStatus('Completed')}>
          Completed
      </button>
  </div>
